...
def main():
    parser = ArgumentParser(description="Gif creator",
                            formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument('-i', '--input', type=Path,
                        help="input file")

    parser.add_argument('-o', '--output', type=Path,
                        help="output file or output folder")
    ...
    parser.add_argument('-ss', '--start', default=DEFAULT_START,
                        help="start time (ffmpeg format): 10, 1:12, 17:5.2, default: %d" % DEFAULT_START)

    parser.add_argument('-t', '--duration', default=DEFAULT_DURATION,
                        help="duration time (ffmpeg format): 10, 1:12, 17:5.2, 0 means all video, default: %d" % DEFAULT_DURATION)
    ...
    parser.add_argument('-c', '--colors', default=DEFAULT_COLORS, type=int,
                        help="video colors, 0 means disable color management, default: %d" % DEFAULT_COLORS)

    parser.add_argument('-r', '--fps', default=DEFAULT_FPS, type=int,
                        help="video frame per second, default: %d" % DEFAULT_FPS)

    parser.add_argument('-O', '--optimize', action='store_true',
                        help="use gifsicle to optimize gif")
    ...
    try:
        ...
        if outputFile.exists():
            ...
            if not args.force:
                return 1
        extractFrames(args.input, workingFolder,
                      args.fps, args.start, args.duration)
        resizeImages(workingFolder,
                     args.width)
