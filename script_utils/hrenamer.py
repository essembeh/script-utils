...
def compute_filename(
    ...
    return (
        (prefix or "")
        + (algo.hexdigest()[0:length] if length > 0 else algo.hexdigest())
        + (suffix or "")
        + (file.suffix if keep_ext else "")
    )
...
def main():
    ...
    for source in visit(args.files, recursive=args.recursive, verbose=args.verbose):
